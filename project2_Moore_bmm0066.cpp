/* File Name: project2_Moore_bmm0066.cpp
* AUTHOR: Blake Moore 
* UserID: bmm0066
* Class: Comp 2710 
* Complie with: g++ project2_Moore_bmm0066.cpp
* Run by: ./a.out
...
* Return: Change A_aliveinto false if Aaron is killed
* Change C_alive into false if Charlie is killed
   */
...
//Runs scenario 1 shootOut
void shootOut1(bool& A_alive, bool& B_alive, bool& C_alive);

//Runs scenario 1 shootOut
void shootOut2(bool& A_alive, bool& B_alive, bool& C_alive);

// Get random number with percent chance as input
// Output is true if it is below the percent, false if above
...
int const TOTAL_RUNS = 10000;
int const AARON_PROB = 33;
int const BOB_PROB = 50;
int percent = 0;
bool aaronAlive, bobAlive, charlieAlive, bob_a, charlie_a, aaron_a;
bool C_alive, B_alive, A_alive = true;
int aaronWins, aaronWins2, bobWins, bobWins2, charlieWins, charlieWins2;



int   main() {
//Initializes Random number generator's seed and calls test functions
   cout << "*** Welcome to Blake Moore's Duel Simulator ***\n";
   srand(time(0));
...
//Starts strategy 1 and runs 10,000 times
   ...
   for (int i = 0; i < TOTAL_RUNS; i++ ){
      shootOut1(A_alive, B_alive, C_alive);
   }
...
//Reinitializes variables and starts strategy 2 to run 10,000 times
   ...
   Press_any_key();
   for (int i = 0; i < TOTAL_RUNS; i++ ){
      shootOut2(A_alive, B_alive, C_alive);
   }
   cout << "Aaron won " << aaronWins2 << "/10000 duels or " << static_cast<double>(aaronWins2) / 
      TOTAL_RUNS * 100 << "%\n"
      << "Bob won " << bobWins2 << "/10000 duels or " << static_cast<double>(bobWins2) / TOTAL_RUNS 
      * 100 << "%\n"
      << "Charlie won " << charlieWins2 << "/10000 duels or " << static_cast<double>(charlieWins2) / 
      TOTAL_RUNS * 100 << "%\n"
      << endl;
...
bool at_least_two_alive(bool A_alive, bool B_alive, bool C_alive) {
   ...
   if (B_alive && (A_alive || C_alive))
   {
      return true;
   }
   if (C_alive && (A_alive || B_alive))
   {
      return true;
   }
   return false;
...
void test_Aaron_shoots1(void) {
   ...
   assert(false == charlie_a || true == charlie_a);
   bob_a = false;
   charlie_a = true;
   ...
   Aaron_shoots1(bob_a, charlie_a);
   assert(false == bob_a || true == bob_a);
}
   
void Bob_shoots(bool& A_alive, bool& C_alive) {
   bool hit = getRandom(50);
   if (hit)
   {
      if (C_alive == true)
      {
...
void test_Bob_shoots(void) {
   ...
   Bob_shoots(aaron_a, charlie_a);
   assert(false == aaron_a || true == aaron_a);
}
void Charlie_shoots(bool& A_alive, bool& B_alive) {
   if (B_alive == true)
   {
